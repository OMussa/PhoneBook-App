const Persons = ({persons, search, handleDelete}) => { //Persons takes in three props but instead of using 'props' its destructured making the code more readable and easier to work with (persons instead of props.persons)
  const filtered = persons.filter(person => person.name && person.name.includes(search)) // filtering out the names from the person arrays using the include method which is looking at whats inputted into the search form. 
